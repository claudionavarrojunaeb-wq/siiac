import { type SyntheticEvent, useEffect, useId, useState, useRef } from "react";
...
export default function InicioForm() {
  ...
  // Asignar el HTML sanitizado al contenedor usando Trusted Types cuando esté disponible.
  useEffect(() => {
      ...
      if (tt && typeof tt.createPolicy === 'function') {
        try {
          policy = tt.createPolicy
            ? tt.createPolicy('siiac-policy', { createHTML: (s: string) => s })
            : null;
        } catch (e) {
          // Si la policy ya existe o falla, intentar obtenerla de nuevo no es crítico.
          policy = null;
        }
      }

      if (policy && typeof policy.createHTML === 'function') {
        // `createHTML` devuelve TrustedHTML; asignamos directamente a innerHTML.
        (campaignRef.current as any).innerHTML = policy.createHTML(campaignHtml);
      } else {
        // Fallback: asignación directa (puede ser bloqueada por Trusted Types en algunos entornos).
        campaignRef.current.innerHTML = campaignHtml;
      }
    } catch (err) {
      console.error('Error asignando campaignHtml al contenedor:', err);
      // Como última medida, usar textContent para no romper la UI.
      if (campaignRef.current) campaignRef.current.textContent = campaignHtml;
    }
  }, [campaignHtml]);
