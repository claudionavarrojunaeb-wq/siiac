...
// Cargar variables desde el .env ubicado en la carpeta del backend
dotenv.config({ path: path.join(__dirname, '.env') });

const PORT = Number(process.env.PORT || 3001);
const ALLOWED_ORIGIN = process.env.CORS_ORIGIN || process.env.FRONTEND_ORIGIN || "http://localhost:5173";
...
// Si el frontend solicita un .mp4 que existe en backend/media, redirigir
// a nuestro endpoint de transcodificación para evitar códecs no soportados.
app.use((req, res, next) => {
  try {
    if (req.method === 'GET' && req.path && req.path.toLowerCase().endsWith('.mp4')) {
      const base = path.basename(req.path);
      const filePath = path.join(__dirname, 'media', base);
      if (fs.existsSync(filePath)) {
        return res.redirect(307, `/api/audio/${encodeURIComponent(base)}`);
      }
    }
  } catch (e) {
    console.error('Error en middleware de redirección .mp4 -> /api/audio:', e);
  }
  next();
});
...
// Permitir la característica WebXR `xr-spatial-tracking` mediante la cabecera
// Permissions-Policy. Esto facilita el desarrollo local donde se usa WebXR.
app.use((req, res, next) => {
  try {
    res.setHeader('Permissions-Policy', 'xr-spatial-tracking=(self)');
  } catch (e) {
    console.error('No se pudo establecer Permissions-Policy header:', e);
  }
  next();
});
...
// Stream de audio con transcodificación on-the-fly usando ffmpeg
// URL: GET /api/audio/:file
// Sirve `${__dirname}/media/:file` reencodificado a MP3 (Content-Type: audio/mpeg)
app.get('/api/audio/:file', (req, res) => {
  try {
    ...
    // Cabeceras para el cliente
    res.setHeader('Content-Type', 'audio/mpeg');
    res.setHeader('Cache-Control', 'no-cache');

    // Spawn ffmpeg para convertir a MP3 y enviar por pipe
    const ff = spawn('ffmpeg', ['-i', filePath, '-vn', '-c:a', 'libmp3lame', '-b:a', '192k', '-f', 'mp3', 'pipe:1']);

    ff.on('error', (err) => {
      console.error('ffmpeg spawn error:', err);
      if (!res.headersSent) res.status(500).json({ ok: false, error: 'Error iniciando ffmpeg' });
      try { ff.kill(); } catch (e) {}
    });

    // Pipe stdout (flujo mp3) hacia la respuesta HTTP
    ff.stdout.pipe(res);

    // Log stderr para depuración
    ff.stderr.on('data', (d) => {
    ...
    console.error('Error en /api/audio/:file', err);
  ...
  try {
    ...
    if (TURNSTILE_SECRET) {
      try {
        ...
        const verifyRes = await axios.post('https://challenges.cloudflare.com/turnstile/v0/siteverify', params.toString(), {
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          timeout: 5000,
        });

        const ok = verifyRes.data && verifyRes.data.success;
        if (!ok) {
          return res.status(401).json({ ok: false, error: 'Turnstile inválido' });
        }
      } catch (e) {
        console.error('Error verificando Turnstile:', e?.response?.data || e.message || e);
        return res.status(500).json({ ok: false, error: 'Error verificando Turnstile' });
      }
    }

    // Intentar bind LDAP con varios formatos de principal
    if (!LDAP_URL) return res.status(500).json({ ok: false, error: 'LDAP no configurado en backend' });

    const client = ldap.createClient({ url: LDAP_URL, timeout: 5000, connectTimeout: 5000 });

    const tryBind = (dn, pw) => new Promise((resolve, reject) => {
    ...
    const principals = [];
    if (LDAP_BASE_DN) {
      principals.push(`uid=${username},${LDAP_BASE_DN}`);
      principals.push(`cn=${username},${LDAP_BASE_DN}`);
    }
    if (LDAP_DOMAIN) {
      principals.push(`${username}@${LDAP_DOMAIN}`);
      principals.push(`${LDAP_DOMAIN}\\${username}`);
    }
    // Always try username as-is last
    principals.push(username);

    let boundPrincipal = null;
    for (const p of principals) {
      try {
        await tryBind(p, password);
        boundPrincipal = p;
        break;
      } catch (e) {
    ...
    if (!boundPrincipal) return res.status(401).json({ ok: false, error: 'Credenciales inválidas' });

    // Autenticación exitosa
    return res.json({ ok: true, principal: boundPrincipal });
  } catch (err) {
