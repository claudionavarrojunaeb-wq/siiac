...
// Endpoint: GET /file
// Query params:
// - name: nombre del fichero (ej. 20260501.jsonl o 20260501.md)
// Responde con descarga del fichero si existe y es un nombre válido.
router.get('/file', (req, res) => {
  try {
    const name = String(req.query.name || '').trim();
    if (!/^\d{8}\.(jsonl|md)$/.test(name)) return res.status(400).json({ ok: false, error: 'invalid file name' });
    ...
    console.error('Error en /api/logsViewer/file', err);
    res.status(500).json({ ok: false, error: String(err) });
  }
});
